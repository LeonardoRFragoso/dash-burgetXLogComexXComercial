...
def custom_round(x):
    frac = x - int(x)
    # Se a parte decimal for estritamente maior que 0.5, arredonda para cima
    if frac > 0.5:
        return int(x) + 1
    else:
        return int(x)
...
if not filtered_df.empty:
    ...
    if not df_current.empty:
        ...
        fig_gap.update_layout(yaxis=dict(autorange="reversed"), height=chart_height)
        # Retirar casas decimais do texto
...
if not filtered_df.empty:
    ...
    if not budget_df.empty:
        ...
        clientes_critico = len(df_graph3[df.graph3['Performance'] < 70])
...
if show_detailed_table and not filtered_df.empty:
    ...
    detailed_df.columns = [
        ...
        'Cabotagem', 'Target Acumulado', 'Realizado (iTracker)', 'Gap de Realização'
    ]
    cols = st.columns([3, 1])
    with cols[0]:
    ...
    with cols[1]:
        sort_by = st.selectbox(
            "Ordenar por",
            options=["Cliente", "Mês", "Budget", "Realizado (iTracker)", "Gap de Realização"],
            index=0
        )
    if search_term:
    ...
    elif sort_by == "Realizado (iTracker)":
        detailed_df = detailed_df.sort_values('Realizado (iTracker)', ascending=False)
    ...
    detailed_df['Realizado (iTracker)'] = detailed_df['Realizado (iTracker)'].apply(lambda x: f'{x:.0f}')
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Realizado (iTracker)": st.column_config.TextColumn("Realizado (iTracker)"),
