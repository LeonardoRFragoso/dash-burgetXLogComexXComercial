...
# =============================================================================
# FUNÇÕES DE DOWNLOAD E COMBINAÇÃO DOS DADOS
# =============================================================================
def download_planilha(file_id):
    """
    Baixa uma planilha do Google Drive dado o file ID.
    """
    try:
        ...
        st.sidebar.info(f"Baixando planilha com ID: {file_id}")
        ...
        st.sidebar.error(f"Erro ao acessar a planilha: {str(e)}")
        return None

@st.cache_data
def load_data():
    # Obter os file IDs dos secrets
    file_id_dados = st.secrets["google"]["file_id_dados_consolidados"]
    file_id_budget = st.secrets["google"]["file_id_budget"]
    file_id_itracker = st.secrets["google"]["file_id_itracker"]  # Use apenas o file ID, sem a URL extra
    
    # Baixar cada planilha
    df_dados = download_planilha(file_id_dados)
    df_budget = download_planilha(file_id_budget)
    df_itracker = download_planilha(file_id_itracker)
    
    if (df_dados is None) or (df_budget is None) or (df_itracker is None):
        st.error("Falha ao carregar todas as planilhas.")
        st.stop()
    
    # Realiza a fusão (merge) com base em 'Cliente' e 'MÊS'
    # Ajuste os nomes das colunas se necessário em cada planilha
    df_temp = pd.merge(df_dados, df_budget, on=["Cliente", "MÊS"], how="outer")
    df = pd.merge(df_temp, df_itracker, on=["Cliente", "MÊS"], how="outer")
    
    # Remove registros sem nome de cliente e converte colunas numéricas
    df = df[df['Cliente'].notna() & (df['Cliente'] != "undefined")]
    numeric_cols = ['BUDGET', 'Importação', 'Exportação', 'Cabotagem', 'Quantidade_iTRACKER', 'Gap de Realização', 'Target Acumulado']
...
with col1:
    ...
    """, unsafe_allow_html=True)
total_oportunidades = (filtered_df['Importação'].sum() + filtered_df['Exportação'].sum() + filtered_df['Cabotagem'].sum()) if all(col in filtered_df.columns for col in ['Importação','Exportação','Cabotagem']) else 0
with col2:
    st.markdown(f"""
    <div class='kpi-card'>
    ...
    """, unsafe_allow_html=True)
total_itracker = filtered_df['Quantidade_iTRACKER'].sum() if 'Quantidade_iTRACKER' in filtered_df.columns else 0
with col3:
    st.markdown(f"""
    <div class='kpi-card'>
...
# Dropdown com lista de clientes (com opção para análise geral)
...
# Define o mês para análise com base na pergunta (ou usa o mês atual)
...
# Prepara o contexto para resposta com base nos dados consolidados
if cliente_mencionado:
    df_cliente = df[(df['Cliente'] == cliente_mencionado) & (df['MÊS'] == mes_corrente)]
    if not df_cliente.empty:
        budget_cliente = df_cliente['BUDGET'].sum() if 'BUDGET' in df_cliente.columns else 0
        realizado_cliente = df_cliente['Quantidade_iTRACKER'].sum() if 'Quantidade_iTRACKER' in df_cliente.columns else 0
        oportunidades_cliente = (df_cliente[['Importação', 'Exportação', 'Cabotagem']].sum().sum() 
                                 if all(col in df_cliente.columns for col in ['Importação','Exportação','Cabotagem']) else 0)
        performance_cliente = (realizado_cliente / budget_cliente) * 100 if budget_cliente > 0 else 0
        gap_cliente = df_cliente['Gap de Realização'].sum() if 'Gap de Realização' in df_cliente.columns else 0
        contexto_cliente = (
...
# Botão para enviar pergunta; não limita o espaço ou tempo de resposta da API
        ...
        with st.spinner("Buscando informações..."):
            if cliente_mencionado and 'df_cliente' in locals() and not df_cliente.empty:
                ...
                sistema_prompt = (
                    ...
                    "Não limite o espaço ou o tempo de resposta para que a análise seja completa. \n\n"
                    f"DADOS DISPONÍVEIS:\n{contexto_cliente}"
                )
                try:
                    resposta = openai.ChatCompletion.create(
                        ...
                        temperature=0.3,
                        max_tokens=1500  # Permite resposta mais extensa se necessário
                    )
...
if not filtered_df.empty:
    ...
    total_budget = filtered_df['BUDGET'].sum() if 'BUDGET' in filtered_df.columns else 0
    total_realizado = filtered_df['Quantidade_iTRACKER'].sum() if 'Quantidade_iTRACKER' in filtered_df.columns else 0
