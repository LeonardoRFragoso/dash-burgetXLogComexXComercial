...
# -------------------------- #
# IMPORTAÇÃO DOS UTILS       #
# -------------------------- #
from utils_dados_clientes import (
...
def load_data():
    ...
    if df is None:
        ...
        st.stop()
    # Verifica se a coluna "Cliente" existe; se não existir mas "CLIENTE (BUDGET)" existir, renomeia.
    if 'Cliente' not in df.columns and 'CLIENTE (BUDGET)' in df.columns:
        df.rename(columns={"CLIENTE (BUDGET)": "Cliente"}, inplace=True)
...
def download_file_from_gdrive():
    try:
        ...
        # Utilizando o ID antigo da planilha de Budget:
...
# Definir o mês consultado para os KPIs – se o usuário selecionar um mês, usa o primeiro; senão, usa o mês atual.
if mes_selecionado:
    mes_consultado = mes_selecionado[0]
else:
    mes_consultado = datetime.now().month

# -------------------------- #
# SEÇÃO DE KPIS             #
# -------------------------- #
# O KPI "Total Budget" considera somente os registros do mês consultado com budget > 0.
total_budget = filtered_df[(filtered_df["MÊS"] == mes_consultado) & (filtered_df["BUDGET"] > 0)]["BUDGET"].sum()
with st.container():
...
# ATUALIZA E CARREGA OS DADOS DO JSON AUXILIAR (usa os defaults: "comparativo_final_atualizado.xlsx" e "dados_clientes_estruturado.json")
dados_estruturados = carregar_dados_estruturados()

if st.button("Enviar Pergunta"):
    if pergunta_input.strip() != "":
        with st.spinner("Buscando informações..."):
            if cliente_mencionado:
                resposta_texto = consultar_dados_cliente(dados_estruturados, cliente_mencionado, mes_corrente)
            else:
                resposta_texto = "Por favor, selecione um cliente para análise com base nos dados registrados."
    ...
    if not df_current.empty:
        ...
        with st.expander("VER DETALHES DO CÁLCULO DESTE GRÁFICO"):
            ...
            **DETALHAMENTO DO CÁLCULO:**
            ...
            - **ORDENAÇÃO:** Os CLIENTES são ordenados de forma decrescente com base no GAP de Realização.
            """)
    else:
    ...
    if not budget_df.empty:
        ...
        with st.expander("VER DETALHES DO CÁLCULO DESTE GRÁFICO"):
            ...
            **DETALHAMENTO DO CÁLCULO:**
            ...
            - **CORES:** Definidas com base nos thresholds (≥100% = SUCESSO, 70-99% = ATENÇÃO, <70% = CRÍTICO).
            """)
...
if not filtered_df.empty:
    ...
    # Utiliza o mês consultado para análise:
    total_budget = filtered_df[(filtered_df["MÊS"] == mes_consultado) & (filtered_df["BUDGET"] > 0)]["BUDGET"].sum()
